...
### 事務 Transaction
事務可以一次執行多個命令， 並且帶有以下幾點重要的保證：
1. 事務是一個單獨的隔離操作：事務中的所有命令都會序列化、按順序地執行。
2. 事務進行的過程中，不會被其他客戶端發送來的命令請求所打斷。
3. 不是原子性，同一個事務中如果有一條執行失敗了，其他的命仍然會執行，沒有回滾。
4. 如果multi開始之後，有語法上的錯誤則`全部`事務都會被取消 (原子性)。
 * ※ 事務的錯誤處理 : 如果事務某階段出錯了，則報錯的命令不會被執行，其他命令都會執行且不會回滾。
 
|-------------指令-------------|------------------------------說明---------------------------------- |--------備註--------|
| --------------------------   |:---------------------------------:									 |:-----			   |
|watch key [key ...]|執行multi前先執行監試一個或多個key，如果執行時發現這些key有被其他命令變更，則執行失敗|返回nil|
|unwatch|取消對所有key的監控|exec、discard執行後也會自動執行unwatch|
|multi|一個事務塊的開始。後續將使用EXEC命令執行排隊等待原子||
...
2. LUA腳本類似redis事務，有一定的原子性，不會被其他命令插隊，可以完成redis的交易。
...
flg := sismember(sk_Prodid , UserID)
if flg != true{
return 此用戶已經參與過此活動 
}
