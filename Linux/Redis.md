...
|zrangebyscore [key] [min] [max] <br />[withscores][limit offset count]|返回有序集key中，所有score介於min和max之間(包刮min、max)的值<br />按照score值有小到大排列。||
|zrevrangbyscore [key] [max] [min] <br />[withscores][limit offset count]|返回有序集key中，所有score介於min和max之間(包刮min、max)的值<br />按照score值有大到小排列。||
